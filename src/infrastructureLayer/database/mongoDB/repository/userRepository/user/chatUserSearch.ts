...
export const chatUserSearch = async (
    query: string
) => {
    const users = await UserModel.find({ "personal_info.username": { $regex: new RegExp(query, "i") } });
    ...
    for (const user of users) {
        ...
        const userObj = {
            ...
            online: true, // Assuming isOnline logic is implemented elsewhere
        };
