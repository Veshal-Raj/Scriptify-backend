...
export const increaseBlogReadCount = async (
  ...
  try {
    ...
    if (blog) {
      const isBlogAlreadyRead = user.userInteractions.userReadBlogs.some(
        (interaction) => interaction.blogId.toString() === blog._id.toString()
      );
      ...
      user.userInteractions.userReadBlogs.push({
